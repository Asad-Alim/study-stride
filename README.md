...
- **Change password** while logged in (`PUT /api/auth/change-password`) — bumps a per-user token version, invalidating every other outstanding session, while the request making the change gets a fresh cookie so it stays logged in
- **Log out other devices** (`POST /api/auth/logout-others`) — invalidates every session except the current one, without requiring a password
...
- Adding a file is **additive** (`POST /api/topics/:id/add-material`): the new material's pages join the pending generation queue (see [Queue-based generation](#queue-based-generation) below) and are ingested into the vector index in the background, without disturbing any notes/sections already generated from earlier material
...
- Before each batch is written to Gemini, `dedupService.js` runs a cheap two-stage check (embedding recall, then a Gemini classification pass only on the candidates that clear that bar) against the topic's already-ingested chunks, and flags any genuine **contradictions** between the new material and what's already been taught so the model can address them explicitly instead of silently teaching two conflicting versions of a concept
...
- Each section can be manually edited inline or **regenerated via AI with feedback**, grounded in the most relevant retrieved-and-reranked chunks of the source material rather than the whole document (RAG — see below)
...
- Descriptive/long/short questions: **AI evaluator grades against the most relevant retrieved-and-reranked chunks** of the source material (RAG-grounded, see below) instead of the entire document, and returns score, feedback, and missing points
- **Section quiz** (`POST /api/quiz/section-quiz`) generates the between-section mini quiz from just that section's content, independent of the standalone quiz page
...
### Ask Anything (cross-topic chat)
- A dedicated `HomeChat` page lets a student ask a question against *all* of their topics at once, not just the one they're currently studying
- `homeChatController.js` embeds the question, runs `$vectorSearch` scoped by `userId` (instead of `topicId`), reranks the candidates, and answers grounded in whatever topic's material is actually relevant
- Each answer is returned with source chips (topic title + snippet) showing which topic and passage it drew from

### Retrieval-Augmented Generation (RAG)
- Every material upload triggers an async, non-blocking `ingestMaterial` job — chunks that one material's pages (`chunkingService.js`, paragraph-aware) and embeds every chunk with Gemini's `gemini-embedding-001` model, independent of concept-section generation pacing
...
- **Retrieve → filter → rerank pipeline** (`retrievalService.js`, tunable in `retrievalConfig.js`): vector search first pulls a wide candidate pool (`CANDIDATE_POOL_SIZE = 20`), those are filtered down to ones above a cosine-similarity floor (`RELEVANCE_THRESHOLD = 0.68`), and the survivors are sent to **Cohere's `rerank-v3.5` model** (`rerankerService.js`, via `https://api.cohere.com/v2/rerank`) to re-score them against the actual query before the top few (`FINAL_CHUNK_COUNT = 5`) are handed to Gemini as context. If the Cohere call fails, `retrievalService.js` falls back to the threshold-filtered, vector-score-ordered list rather than failing the request outright.
- **Two different AI-context strategies, used deliberately**:
  ...
  - *Grounded single-answer tasks* (section chat, learning-mode "ask", quiz answer evaluation, notes editing, Ask Anything) instead go through the retrieve-filter-rerank pipeline above to get only the top-k most relevant chunks, since these need precision and low latency more than exhaustive coverage.
...
- `POST /api/auth/login` and `POST /api/auth/register` set a `Set-Cookie: token=...; HttpOnly; SameSite=Strict (prod) / Lax (dev); Secure (prod)` header instead of returning `{ token }` in the body
...
- Since there's no longer a token the frontend can inspect to know "am I logged in", `AuthContext` always calls `GET /api/auth/me` on mount and treats a 401 as "not logged in", rather than gating that call on a `localStorage` check
...
| AI (reranking) | Cohere Rerank (`rerank-v3.5`, called directly via `fetch` against `api.cohere.com/v2/rerank` — no SDK dependency) |
...
| Rate Limiting | `express-rate-limit` (fixed-window, for auth/upload/general routes) + a hand-rolled sliding-window-counter limiter (for Gemini generation routes) |
...
└── study stride/
    ...
    │   │   ├── topicController.js        # CRUD for topics, queueStatus, updateStrictMode, cascade delete
    │   │   ├── materialController.js     # File upload, text extraction, Cloudinary, cascade delete, stale-section flagging
    │   │   ├── learningController.js     # Legacy chunk delivery / ask-question controller (route currently unmounted — see server.js)
    ...
    │   │   ├── sectionController.js      # ConceptSection CRUD, queue-based generateNextBatch (runs dedup check), RAG-backed section chat
    ...
    │   │   ├── Topic.js                  # combinedText, conceptIndex, strictMode, learningSections cache
    ...
    │   │   ├── topicRoutes.js            # + POST /:id/add-material (additive, queue-friendly, triggers ingestMaterial)
    │   │   ├── materialRoutes.js         # POST / (upload, up to 10 files), GET /, GET /:id, DELETE /:id
    │   │   ├── learningRoutes.js         # Legacy routes — defined but not mounted in server.js
    │   │   ├── notesRoutes.js            # generate-next-batch, queue-status, save/save-section, update-with-ai, generate-and-get, approve
    │   │   ├── flashcardRoutes.js        # + GET /:materialId/stale-check
    │   │   ├── quizRoutes.js             # + GET /:materialId/stale-check, POST /section-quiz
    │   │   ├── generationRoutes.js       # + GET /:materialId/stale-check
    │   │   ├── sectionRoutes.js          # chat, complete, edit, update-note, regenerate-stale, lock, lock-all
    │   │   └── homeChatRoutes.js         # POST /ask — cross-topic "Ask Anything" chat
    │   ├── services/
    ...
    │   │   ├── chunkingService.js        # Paragraph-aware chunker
    ...
    │   │   ├── retrievalConfig.js        # RELEVANCE_THRESHOLD / CANDIDATE_POOL_SIZE / FINAL_CHUNK_COUNT constants
    │   │   ├── retrievalService.js       # retrieveRelevantChunks — search + threshold filter + Cohere rerank, with fallback
    │   │   ├── rerankerService.js        # rerankChunks — calls Cohere's rerank-v3.5 API directly
    │   │   └── dedupService.js           # runDedup — embedding-recall + Gemini-classification contradiction check for new material
    │   └── server.js                     # Express app, cookie-parser, global apiLimiter, routes, global error handler
    └── frontend/
        └── src/
            ├── api/
            │   ├── axios.js               # withCredentials: true, 401 → redirect to /login
            │   └── homeChat.js            # askHomeChat — calls POST /api/home-chat/ask
            ├── components/
            ...
            │   ├── TopicFiles.jsx         # View/add/delete files in a topic, new-material heads-up popup
            │   ├── LearningMode.jsx       # Full learning interface (2 or 3 panel)
            │   ├── Notes.jsx              # Standalone notes viewer/editor, continue-generating queue UI
            │   ├── Flashcards.jsx         # Flashcard grid, staleness banner + regenerate
            │   ├── Quiz.jsx               # Full quiz page with AI evaluation, staleness banner + regenerate
            │   ├── GeneratedContent.jsx   # Summary / Revision / Cheat Sheet tabs, staleness banner + regenerate
            │   ├── HomeChat.jsx           # Cross-topic "Ask Anything" chat, with per-message source chips
            │   ├── Profile.jsx            # Profile view/edit, change-password, log-out-other-devices
...
- A **Cohere API key** (for the reranking step in the retrieval pipeline — free tier is sufficient for development)
...
- **`combinedText` architecture**: All files in a topic are concatenated into one field on `Topic`. Bulk-generation calls read from this — no chunking, no risk of a concept being split across files or pages. Chat/grounded tasks use the RAG pipeline instead (see [RAG](#retrieval-augmented-generation-rag)).
...
- **Vector search first, rerank second**: `$vectorSearch` is fast but its cosine-similarity ranking is a coarse approximation of "relevant to this exact question." Pulling a wide candidate pool from Atlas and then having Cohere's cross-encoder rerank model re-score the pool against the literal query text catches cases the embedding similarity alone would rank poorly, at the cost of one extra (cheap, fast) API call per grounded request. If that call fails, the pipeline degrades gracefully to the vector-score order rather than erroring out.
...
- **Contradiction detection before generation, not after**: `dedupService.js` runs a cheap embedding-recall pass (low threshold, on purpose — it's a candidate filter, not the verdict) against the topic's already-ingested chunks, then only pays for a Gemini classification call on the pairs that clear that bar. Only genuine contradictions are surfaced into the generation prompt; duplicate or merely-extending content is already handled by the `conceptIndex` one-liners, so it isn't worth flagging again.
...
- `learningController.js`/`learningRoutes.js` still exist in the codebase but are commented out in `server.js` — legacy code that hasn't been removed yet